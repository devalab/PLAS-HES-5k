       ...
       for line in id_list:
           ...
           if ligand_status(index,pdbid):
               ...
               for run in range(len(frames)):
                   ...
                   #os.system(f'cp {scipt_dir}/cpptraj_2.in {cur_dir}/')
                   #os.system(f'cp {scipt_dir}/cpptraj_200.in {cur_dir}/')
                   #os.system(f'cp {scipt_dir}/mmpbsa.in {cur_dir}/')
                   #os.system(f'cp {scipt_dir}/water_selection_no_skip.py {cur_dir}/')
                   cpptraj_starting = f"""
